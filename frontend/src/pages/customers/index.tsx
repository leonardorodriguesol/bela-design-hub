      ...
      <header className="rounded-3xl border border-brand-100 bg-white/95 p-6 text-brand-700 shadow-sm">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-semibold text-brand-800">Base de clientes</h2>
            <p className="text-sm text-brand-500">
              Cadastro centralizado para pedidos e ordens de serviço.
            </p>
            <p className="text-xs text-brand-400">
              {isLoading
                ? 'Carregando registros...'
                : `${totalCustomers.toLocaleString('pt-BR')} cliente(s) cadastrados`}
            </p>
          </div>

          <div className="flex w-full max-w-md flex-col gap-3">
            <label className="flex flex-col text-sm text-brand-500">
              <span className="mb-1 font-medium text-brand-700">Filtrar por nome</span>
              <input
                type="text"
                className="rounded-xl border border-brand-100 bg-white px-4 py-2.5 text-brand-700 placeholder:text-brand-300 focus:border-brand-500 focus:outline-none"
            ...
            {!isLoading && !error &&
              filteredCustomers.map((customer) => (
                <tr key={customer.id} className="border-t border-brand-100">
                  <td className="px-6 py-4 text-sm font-medium text-brand-800 whitespace-nowrap">{customer.name}</td>
                  <td className="px-6 py-4 text-brand-600">{customer.email ?? '—'}</td>
                  <td className="px-6 py-4 text-brand-600 whitespace-nowrap">{customer.phone ?? '—'}</td>
